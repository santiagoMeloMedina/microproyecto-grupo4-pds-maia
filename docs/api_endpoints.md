...
## GET `/api/v1/schedule-slots/breakdown`

**Qué hace:** agrupa la tasa de retraso y el volumen de vuelos por aerolínea, franja horaria o día de la semana, dentro de la selección activa. Útil para gráficos de barras tasa-vs-volumen.

**Query params:**
- `by` (string, **requerido**): uno de `"airline"`, `"slot"`, `"day"`.
- `airline`, `route`, `dayOfWeek` (1–7), `slot`: filtros opcionales (igual que en `/schedule-slots`).
- `top` (int, 1–50, opcional): limita a las N categorías con más vuelos.

**Ejemplo de request:**
```
GET /api/v1/schedule-slots/breakdown?by=day&airline=WN&top=7
```

**Respuesta 200** (lista de `SlotsBreakdownItem`):
```json
[
  { "key": "5", "label": "Viernes", "rate": 0.44, "flights": 3200 },
  { "key": "3", "label": "Miercoles", "rate": 0.4, "flights": 2980 }
]
```

**Errores:**
- `400 Bad Request` si `by` no es uno de los valores permitidos (aunque `Query(pattern=...)` ya lo valida antes con un `422`):
  ```json
  { "detail": "'by' debe ser uno de ['airline', 'day', 'slot']" }
  ```
...
**Ejemplo de request:**
```
GET /api/v1/schedule-slots/drift?route=DAL-HOU
```
