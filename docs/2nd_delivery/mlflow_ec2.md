...
## 6. Qué mirar en la UI

Los experimentos quedan bajo `airlines-retrasos`, etiquetados por `familia` y `etapa`:

| Etiqueta | Contenido |
|---|---|
| `etapa=busqueda` | Las 90 corridas de búsqueda, con métricas de **validación** (días 20–24) |
| `etapa=final` | Los 3 finalistas reajustados sobre días 0–24, con métricas de **prueba** (días 25–30) |
| `etapa=registro` | El modelo elegido, con su artefacto y su entrada en el Model Registry |

Filtros útiles en la barra de búsqueda de la UI:

```
tags.etapa = "busqueda" and metrics.roc_auc > 0.69
tags.familia = "xgboost"
```

Para comparar: seleccionar varias corridas → **Compare** → gráfico de coordenadas paralelas, que
muestra de un vistazo qué hiperparámetros llevan a mejor AUC.

## 7. Capturas requeridas por la entrega

La Entrega 2 pide explícitamente: *"Pantallazos de experimentos registrados en MLflow en una máquina
de AWS EC2 (debe ser visible el usuario e IP de la máquina en EC2, y la IP en MLflow)"*.

Guardar los archivos en **`docs/2nd_delivery/images/`** con estos nombres exactos, que son los que
el reporte ya referencia en su sección 7.

| # | Archivo | Qué debe verse | Dónde tomarla |
|---|---|---|---|
| 1 | `mlflow_01_instancia_ec2.png` | ID de la instancia, **IP pública IPv4**, estado *Running*, tipo de instancia | Consola de AWS → EC2 → Instances → tu instancia → pestaña *Details* |
| 2 | `mlflow_02_servidor_ssh.png` | El prompt con **usuario y host** (`ubuntu@ip-172-31-x-x`), la salida de `mlflow --version` y el servidor activo | Terminal SSH conectada a la instancia |
| 3 | `mlflow_03_experimento.png` | **La barra de direcciones con `http://<ip-publica>:5000`** y el experimento `airlines-retrasos` con sus corridas | Navegador, UI de MLflow |
| 4 | `mlflow_04_comparacion.png` | Varias corridas seleccionadas y comparadas, con la IP visible en la URL | UI de MLflow → seleccionar corridas → *Compare* |
| 5 | `mlflow_05_corrida_detalle.png` | Parámetros y métricas de una corrida (`roc_auc`, `brier`, `tamano_mb`…) | UI de MLflow → clic en una corrida |
| 6 | `mlflow_06_modelo_registrado.png` | El modelo `riesgo-retraso-vuelos` versión 1 | UI de MLflow → *Models* |

**Para que la captura 2 sirva como evidencia**, conviene ejecutar antes estos comandos en la sesión
SSH, de modo que el pantallazo muestre a la vez el usuario, el host y que el servidor responde:
...
**Antes de tomar las capturas 3 a 6** hay que haber ejecutado el notebook apuntando a la instancia,
de modo que el experimento tenga las 90 corridas de búsqueda, los 3 finalistas, el *ablation* y el
registro del modelo. Los filtros útiles en la barra de búsqueda de la UI:

```
tags.etapa = "busqueda"
tags.etapa = "final"
tags.familia = "xgboost" and metrics.roc_auc > 0.70
```

> **Al terminar: detener la instancia, no terminarla.** La entrega pide explícitamente mantenerla
> detenida. En la consola de EC2: *Instance state → Stop instance* (nunca *Terminate*).

## 8. Nota sobre credenciales

Si la instancia vive en AWS Academy Learner Lab, la sesión caduca y la IP pública cambia al
reiniciar. Conviene asignar una IP elástica o volver a exportar `MLFLOW_TRACKING_URI` en cada
sesión. Para algo estable, la alternativa es una cuenta personal en capa gratuita, como ya se
documentó para el bucket de DVC en [`scripts/windows/README.md`](../../scripts/windows/README.md).
